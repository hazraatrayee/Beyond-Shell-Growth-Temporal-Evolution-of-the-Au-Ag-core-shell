...
fclose(fr2);
fclose(fr1);
/*writing modified intermediate shell refractive index for domain2*/
fr1=fopen("modified_inter_shell.txt","r");
for(i=0;i<=l;i++)
{
for(j=0;j<=2;j++)
fscanf(fr1,"%f",&b[l][j]);
...
fclose(fr2);
fclose(fr1);
/*writing modified core refractive index for domain3*/
