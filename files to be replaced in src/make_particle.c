...
// shape parameters
static double coat_x,coat_y,coat_z,coat_r2,coat_rI,coat_rII,coat_r;
 ...
 */
{
	...
	switch (shape) {
		...
		case SH_BISHELL:  {
			double coat_ratioI, coat_ratioII;
			coat_ratioI=sh_pars[0];
			coat_ratioII=sh_pars[1];
			TestRangeII(coat_ratioI,"inner/outer diameter ratio",0,1);
			if(IFROOT){
			sh_form_str1="bishell; diameter(d):";
			sh_form_str2=dyn_sprintf(", core diameter d_in/d="GFORM,coat_ratioII);
			}
			coat_rI=0.25*coat_ratioI*coat_ratioI;
			coat_rII=0.25*coat_ratioII*coat_ratioII;
		...
		case SH_SPHERICALHOLLOW: {
			double coat_ratioh;
			coat_ratioh=sh_pars[0];
			TestRangeII(coat_ratioh,"inner/outer diameter ratio",0,1);
			if(IFROOT) {
				sh_form_str1="sphericalhollow; diameter(d):";
				sh_form_str2=dyn_sprintf(", hollow core diameter d_in/d="GFORM, coat_ratioh);
			}
			coat_r=0.25*coat_ratioh*coat_ratioh;
			volume_ratio=PI_OVER_SIX;
			yx_ratio=zx_ratio=1;
			Nmat_need=1;
			break;
	...
	for(k=local_z0;k<local_z1_coer;k++) for(j=0;j<boxY;j++) for(i=0;i<boxX;i++) {
		...
		switch (shape) {
			...
			case SH_BIELLIPSOID:
				...
				break;
			case SH_BISHELL:
				if (xr*xr+yr*yr+zr*zr<=coat_rII) mat=2;
				else{
					if (xr*xr+yr*yr+zr*zr>=coat_rII && xr*xr+yr*yr+zr*zr<=coat_rI) mat=1;
					if (xr*xr+yr*yr+zr*zr>=coat_rI && xr*xr+yr*yr+zr*zr<=0.25) mat=0;
					}
				break;
			case SH_BISPHERE:
				ro2=xr*xr+yr*yr;
			...
			case SH_SPHEREBOX:
				...
				break;
			case SH_SPHERICALHOLLOW:
				if (xr*xr+yr*yr+zr*zr>=coat_r && xr*xr+yr*yr+zr*zr<=0.25) mat=0;
				break;
		}
		/* TO ADD NEW SHAPE
